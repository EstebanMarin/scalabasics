...
class SetSuite extends AnyFunSuite with Matchers {
  ...
  test("union with variance") {
    ...
    Set[Employee](employee)
      .add(consultant: CompanyRole) shouldBe Set[CompanyRole](employee, consultant)
  }
  ...
  test(
    "toString on a Set with two elements should contain 2 braces, both elements, 2 parens and one comma"
  ) {
    val first = randomString
    val second = randomString
  ...
  test(
    "toString on a Set with two elements should contain 2 braces, both elements, 2 parens and two commas"
  ) {
    val first = randomString
    val second = randomString
