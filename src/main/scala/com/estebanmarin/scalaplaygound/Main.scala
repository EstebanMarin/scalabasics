...
object Main {
  ...
  private def runDemo(): Unit = {
    ...
    val invalidCard: CreditCard =
      CreditCard(
        "1234567812345678"
      ) // change the check digit (the last digit) to zero to make it a valid card
    ...
    println(
      "You can also pass in the credit card number as a command line arugment like this: run 5893505008915446"
    )
  }
}
